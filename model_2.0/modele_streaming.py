...
EMB_DIM = 64  # Taille du vecteur embedding pour chaque mot/token. Exemple: un token devient un vecteur de 64 nombres
HIDDEN_DIM = 128  # Taille de l'etat cache du LSTM. Exemple: sortie interne de taille 128
CONTEXT_SIZE = 8  # Nombre de tokens utilises pour predire le suivant. Exemple: [10, 11, 12] -> predire token suivant
BATCH_SIZE = 2  # Nombre d'exemples traites en meme temps. Exemple: 2 sequences par passage
CHUNK_TOKENS = 5_000  # Nombre de tokens charges par morceau (chunk). Exemple: lire 5000 IDs puis entrainer
EPOCHS = 30  # Nombre de passages complets d'entrainement. Exemple: 1 = un seul tour sur les donnees lues
MAX_TOKENS_PER_EPOCH = 100_000  # Limite de tokens lus par epoch. Exemple: arret apres 100000 tokens

# Reprise d'entrainement (resume). Exemple: si True et que le fichier existe, on recharge les poids avant de re-entrainer.
BASE_DIR = ""  # Dossier Drive optionnel. Exemple: "/content/drive/MyDrive"
...
DATASET_IDS_PATH = os.path.join(DATA_DIR, "dataset-i.txt")  # Chemin du fichier d'IDs. Exemple: "/content/drive/MyDrive/dataset-i.txt"
...
CHECKPOINT_LOAD_PATH = os.path.join(SCRIPT_DIR, "modele_streaming.pt")  # Chemin du modele a CHARGER (lecture). Exemple: "C:/.../modele_streaming.pt"
CHECKPOINT_SAVE_PATH = os.path.join(SCRIPT_DIR, "modele_streaming.pt")  # Chemin du modele a SAUVEGARDER (ecriture). Exemple: "C:/.../modele_streaming.pt"
...
def prediction(X_batch, y_batch, emb_layer, lstm, fc, optimizer, criterion):  # Fait un passage avant + arriere pour un batch. Exemple: une etape d'apprentissage
    # deplacer les batches sur le meme device que le modele (GPU si present)
    device = next(emb_layer.parameters()).device
    X_batch = X_batch.to(device)
    y_batch = y_batch.to(device)

    vecs = embedding(X_batch, emb_layer)  # Convertit IDs en vecteurs. Exemple: [B, T] -> [B, T, EMB_DIM]
    out, _ = lstm(vecs)  # Envoie les vecteurs dans le LSTM. Exemple: sortie [B, T, HIDDEN_DIM]
    last = out[:, -1, :]  # Garde seulement la derniere sortie temporelle. Exemple: [B, HIDDEN_DIM]
...
def save_checkpoint(path, emb_layer, lstm, fc, optimizer=None, meta=None):
...
def train_streaming(  # Fonction principale d'entrainement en lecture progressive (streaming). Exemple: train sans charger tout le dataset
    ...
    lstm,  # Reseau recurrent LSTM. Exemple: nn.LSTM(...)
        ...
        if len(buffer_ids) >= chunk_tokens:  # Si le tampon a atteint la taille chunk, on entraine dessus. Exemple: 5000 tokens
            ...
            if len(buffer_ids) > context_size:  # Verifie qu'on a assez de tokens pour creer au moins un exemple. Exemple: >3
                ...
                for batch_index, (X_batch, y_batch) in enumerate(loader, start=1):  # Parcourt chaque mini-lot du chunk. Exemple: batch 1, batch 2, ...
                    loss, pred_ids = prediction(X_batch, y_batch, emb_layer, lstm, fc, optimizer, criterion)  # Fait l'apprentissage du mini-lot et recupere les IDs predits
            ...
            if checkpoint_path:  # Sauvegarde un checkpoint apres chaque chunk pour ne pas perdre la progression. Exemple: modele_streaming.pt
                ...
                save_checkpoint(checkpoint_path, emb_layer, lstm, fc, optimizer=optimizer, meta=meta)  # Ecrit le fichier .pt sur disque
    ...
    if len(buffer_ids) > context_size:  # Apres la boucle, traite le dernier morceau restant. Exemple: fin de fichier
        ...
        for batch_index, (X_batch, y_batch) in enumerate(loader, start=1):  # Entraine sur ce dernier morceau. Exemple: boucle finale
            loss, pred_ids = prediction(X_batch, y_batch, emb_layer, lstm, fc, optimizer, criterion)  # Mise a jour du modele avec recuperation des IDs predits
        ...
        if checkpoint_path:  # Sauvegarde aussi le dernier morceau final si present. Exemple: reste de fin de fichier
            ...
            save_checkpoint(checkpoint_path, emb_layer, lstm, fc, optimizer=optimizer, meta=meta)  # Ecrit le checkpoint final de chunk
...
if __name__ == "__main__":
    print("debut")
    print("nouveau datset")  # Ce bloc s'execute seulement si on lance ce fichier directement. Exemple: python modele_streaming.py
  ...
  # Message pour voir que le script a bien demarre. Exemple affiche: debut
    ...
    hidden_dim = HIDDEN_DIM  # Taille cache LSTM locale. Exemple: 128
    ...
    lstm = nn.LSTM(input_size=emb_dim, hidden_size=hidden_dim, batch_first=True)  # Cree le LSTM. Exemple: entree [B,T,64] sortie [B,T,128]
    fc = nn.Linear(hidden_dim, vocab_size)  # Cree la couche de classification finale. Exemple: [128] -> [10000 scores]
    ...
    optimizer = torch.optim.SGD(  # Cree l'optimiseur SGD pour entrainer les 3 blocs du modele. Exemple: lr=0.05
        list(emb_layer.parameters()) + list(lstm.parameters()) + list(fc.parameters()),  # Liste tous les poids a mettre a jour
        lr=0.05,  # Taux d'apprentissage. Exemple: 0.05
    ...
    # Detecte le device: GPU si disponible, sinon CPU
    ...
    if RESUME_IF_CHECKPOINT_EXISTS and os.path.exists(checkpoint_load_path):  # Si on veut reprendre et que le fichier existe. Exemple: True + fichier present
        try:  # On tente de charger (tailles differentes/corruption peuvent echouer). Exemple: try/except
            ...
            lstm.load_state_dict(ckpt["lstm"])  # Recharge les poids du LSTM. Exemple: reprend l'apprentissage
            ...
            if "optimizer" in ckpt:  # Si l'optimiseur a ete sauvegarde aussi. Exemple: nouveau format de checkpoint
                optimizer.load_state_dict(ckpt["optimizer"])  # Recharge l'etat optimiseur. Exemple: resume plus fidele
    ...
    for epoch in range(epochs):  # Boucle sur le nombre d'epochs. Exemple: --epochs 5 -> 5 iterations
        ...
        loss_moyenne = train_streaming(  # Lance l'entrainement streaming pour une epoch. Exemple: retourne perte moyenne
            ...
            lstm,  # LSTM a entrainer. Exemple: lstm
    ...
    out, _ = lstm(vecs)  # Passe les vecteurs dans le LSTM. Exemple: [1,3,64] -> [1,3,128]
    ...
    print("lstm out shape:", out.shape)  # Affiche la taille de sortie LSTM de test. Exemple: torch.Size([1, 3, 128])
    ...
    if PRINT_PREDICTED_TEXT:  # Si on veut afficher une phrase lisible. Exemple: True
        ...
        lstm.eval()  # Mode evaluation. Exemple: stable pour generation
        ...
        with torch.no_grad():  # Pas de gradients pendant la generation. Exemple: plus rapide et moins RAM
            for _ in range(GENERATE_TOKENS):  # Genere plusieurs tokens. Exemple: 20
                ...
                out, _ = lstm(vecs)  # LSTM. Exemple: [1,8,64] -> [1,8,128]
...
# - Etape E: LSTM lit la sequence de vecteurs pour comprendre l'ordre dans le temps.
# - Etape F: couche fc transforme la sortie LSTM en scores pour tous les IDs possibles.
...
#   - il contient les poids de emb, lstm et fc.
# - Sortie test final:
...
#   - tailles affichees: vecs shape et lstm out shape.
#
...
# - INPUT intermediaire: X_batch/y_batch -> passe dans embedding, puis LSTM, puis fc.
