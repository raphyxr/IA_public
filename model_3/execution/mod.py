...
class modele:
    ...
    def charger_modele():
        emb_layer, pos_embedding, transformer, fc, optimizer, criterion = ms.init_training_objects(vocab_size=100000)
        ms.load_checkpoint_if_compatible("model_V2.1.pt", emb_layer, pos_embedding, transformer, fc)
        return emb_layer, transformer, fc

    def gen(emb_layer, transformer, fc, tensor):
        emb = ms.embedding(tensor ,emb_layer)
        out, hidden = ms.transformer(emb)
        last = out[:, -1,:]
        fcs = fc(last)
    ...
    def generation (self, promp, jsons):
        emb_layer, transformer, fc = modele.charger_modele()
        generation_txt = True
        x = 0
        gener = []
        ...
        while generation_txt == True:           
            gen = modele.gen(emb_layer, transformer, fc, tens)
            gen = gen[0].item()
            x += 1
